...
module.exports.handleEvent = async function({ api, event, Currencies, Users, getText }) {
    ...
    if (level > curLevel && level != 1) {
        ...
        if (existsSync(__dirname + `/cache/rankup/rankup.mp4`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/rankup.gif`), mentions: [{ tag: name, id: senderID }] };
...
module.exports.languages = {
    "vi": {
        ...
        "levelup": "{name} đã đạt tới level {level} cố gắng tương tác hơn nữa nàooo!!!"
    },
    "en": {
        "on": "on",
        "off": "off",
